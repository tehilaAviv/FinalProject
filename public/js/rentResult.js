var currentUserForRent = {}
var images={}

function getUserRent() {
    fetch("/dbReturn/returnUserRent")
        .then(response => response.json())
        .then(data => {
            currentUserForRent = data
        })
        .catch(console.error);
}
//

function showUserRent() {
    ...
    fetch("/dbReturn/returnUserRent/")
        ...
        .then(data => {
            ...
            html += '<h4>   כאן מופיעות המודעות שפרסמת לשכירה, להצגת תוצאות לחץ על כפתור הצג תוצאות </h4>\n';
            html += '</div>';
            html += '</div>';
            ...
            for (const product of data) {
                ...
                html += '                                            <button type=\"button\" onclick="showLandlord(\'' + product.id+ '\')"  class=\"btn btn-sm btn-outline-info\">הצג תוצאות</button>\n';
                html += '                                        </div>\n';
...
function showLandlord(id) {

    let myList = document.getElementById("show");
    if (currentUserForRent != undefined) {
        for (const index of currentUserForRent) {
            if(index.id==id)
                var id_=index;
        }
        fetch("/dbReturn/returnLandlord/"+id, {
            method: 'POST',
            ...
            .then(data => {
                if(data.length==0)
                    ...
                    for (const product of data) {
                        ...
                        html += "<div class='text'> " + "<b>מספר תשלומים בשנה:</b>" + " " + product.Number_of_payments + "</div>";
    ...
    fetch("/dbReturn/returnUserRent/")
        ...
        .then(data => {
            ...
            for (const product of data) {
                ...
                html += ' <a  href="\dbEdit/eraseRent/' + product.id+ ' ">'
                html += '                                            <button type=\"button\" class=\"btn btn-sm btn-outline-info\">מחק</button>\n';
                html += ' </a>'
                html += ' <a  href="\dbEdit/rent/' + product.id+ ' ">'
